...
public class TranscodeBackground extends AsyncTask<Void, Integer, Integer>
{
	...
	public TranscodeBackground(BaseWizard act, IFfmpgefRemoteServiceBridge remote, int notificationId) {
		...
		_prefs = new Prefs();
		_prefs.setContext(_act.cordova.getActivity().getApplicationContext());
	...
	protected void onPreExecute() {
		PowerManager powerManager = (PowerManager)_act.cordova.getActivity().getSystemService(_act.cordova.getActivity().POWER_SERVICE);
		...
		progressDialog = new ProgressDialog(_act.cordova.getActivity().getApplicationContext());
		...
		try {
			object.put("statues", "Started");
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		
		PluginResult progressResult = new PluginResult(	PluginResult.Status.OK, object.toString());
		progressResult.setKeepCallback(true);
		_act.callbackContext.sendPluginResult(progressResult);*/
	...
	protected void onProgressUpdate(Integer... progress) {
		...
		/*JSONObject object=new JSONObject();
		try {
			object.put("statues", "progressing");
			object.put("progress", p);
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	protected void onPostExecute(Integer result) {
		...
		if (_act != null) {
			...
			/*JSONObject object=new JSONObject();
			try {
				object.put("statues", "stoped");
				object.put("path", "/sdcard/videokit/final.mp4");
			} catch (JSONException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	private void showFinishNotification () {
		...
		NotificationManager nm = (NotificationManager) _act.cordova.getActivity().getSystemService(_act.cordova.getActivity().NOTIFICATION_SERVICE);
		...
		if (Prefs.forceStopFlag) {
			...
			notification.setLatestEventInfo(_act.cordova.getActivity().getApplicationContext(), notifTitle, "", pIntent);
			...
			if (_act.getOutputFile() == null) {
				...
				case Prefs.FILE_TYPE_VIDEO:
					...
					ContentResolver resolver = _act.cordova.getActivity().getApplicationContext().getContentResolver();
	        	    ...
	        	    if (uri == null) {
	        	    	...
	        	    	String cUri = FileUtils.getVideoContentUriFromFilePath(_act.cordova.getActivity().getApplicationContext(), outputFileURL);
	        	    	if (cUri != null) {
	        	    		uri = Uri.parse(cUri);
				...
				notification.setLatestEventInfo(_act.cordova.getActivity().getApplicationContext(), title,
						desc,
						pIntent);
			}
